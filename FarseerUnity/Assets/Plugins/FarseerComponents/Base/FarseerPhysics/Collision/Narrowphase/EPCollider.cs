...
namespace VelcroPhysics.Collision.Narrowphase
{
    public static class EPCollider
    {
        public static void Collide(ref Manifold manifold, EdgeShape edgeA, ref Transform xfA, PolygonShape polygonB, ref Transform xfB)
        {
            ...
            FVector2 normal;
            FVector2 normal0 = FVector2.Zero;
            FVector2 normal2 = FVector2.Zero;

            Transform xf = MathUtils.MulT(xfA, xfB);

            FVector2 centroidB = MathUtils.Mul(ref xf, polygonB.MassData.Centroid);

            FVector2 v0 = edgeA.Vertex0;
            FVector2 v1 = edgeA._vertex1;
            FVector2 v2 = edgeA._vertex2;
            FVector2 v3 = edgeA.Vertex3;

            bool hasVertex0 = edgeA.HasVertex0;
            bool hasVertex3 = edgeA.HasVertex3;

            FVector2 edge1 = v2 - v1;
            edge1.Normalize();
            FVector2 normal1 = new FVector2(edge1.Y, -edge1.X);
            float offset1 = FVector2.Dot(normal1, centroidB - v1);
            ...
            FVector2[] normals = new FVector2[Settings.MaxPolygonVertices];
            FVector2[] vertices = new FVector2[Settings.MaxPolygonVertices];
            ...
            {
                float s = FVector2.Dot(normal, vertices[i] - v1);
            ...
            FVector2 perp = new FVector2(-normal.Y, normal.X);

            for (int i = 0; i < count; ++i)
            {
                FVector2 n = -normals[i];

                float s1 = FVector2.Dot(n, vertices[i] - v1);
                float s2 = FVector2.Dot(n, vertices[i] - v2);
                ...
                {
                    if (FVector2.Dot(n - upperLimit, normal) < -Settings.AngularSlop)
                    {
                        continue;
                    }
                }
                ...
                {
                    if (FVector2.Dot(n - lowerLimit, normal) < -Settings.AngularSlop)
                    {
                        continue;
                    }
                }
            ...
            if (primaryAxis.Type == EPAxisType.EdgeA)
            {
                ...
                int bestIndex = 0;
                float bestValue = FVector2.Dot(normal, normals[0]);
                for (int i = 1; i < count; ++i)
                {
                    float value = FVector2.Dot(normal, normals[i]);
                    if (value < bestValue)
                    {
                        bestValue = value;
                        bestIndex = i;
            ...
            {
                float separation = FVector2.Dot(rf.Normal, clipPoints2[i].V - rf.v1);
