...
namespace VelcroPhysics.Collision.Narrowphase
{
    public static class CollidePolygon
    {
        ...
        public static void CollidePolygons(ref Manifold manifold, PolygonShape polyA, ref Transform xfA, PolygonShape polyB, ref Transform xfB)
        {
            ...
            FVector2 v11 = vertices1[iv1];
            FVector2 v12 = vertices1[iv2];

            FVector2 localTangent = v12 - v11;
            localTangent.Normalize();

            FVector2 localNormal = MathUtils.Cross(localTangent, 1.0f);
            FVector2 planePoint = 0.5f * (v11 + v12);

            FVector2 tangent = MathUtils.Mul(ref xf1.q, localTangent);
            FVector2 normal = MathUtils.Cross(tangent, 1.0f);

            v11 = MathUtils.Mul(ref xf1, v11);
            v12 = MathUtils.Mul(ref xf1, v12);

            // Face offset.
            float frontOffset = FVector2.Dot(normal, v11);

            // Side offsets, extended by polytope skin thickness.
            float sideOffset1 = -FVector2.Dot(tangent, v11) + totalRadius;
            float sideOffset2 = FVector2.Dot(tangent, v12) + totalRadius;
            ...
            {
                float separation = FVector2.Dot(normal, clipPoints2[i].V) - frontOffset;
        ...
        private static float FindMaxSeparation(out int edgeIndex, PolygonShape poly1, ref Transform xf1, PolygonShape poly2, ref Transform xf2)
        {
            ...
            for (int i = 0; i < count1; ++i)
            {
                ...
                FVector2 n = MathUtils.Mul(ref xf.q, n1s[i]);
                FVector2 v1 = MathUtils.Mul(ref xf, v1s[i]);

                // Find deepest point for normal i.
                float si = Settings.MaxFloat;
                for (int j = 0; j < count2; ++j)
                {
                    float sij = FVector2.Dot(n, v2s[j] - v1);
                    if (sij < si)
                    {
                        si = sij;
                    }
        ...
        private static void FindIncidentEdge(out FixedArray2<ClipVertex> c, PolygonShape poly1, ref Transform xf1, int edge1, PolygonShape poly2, ref Transform xf2)
        {
            ...
            FVector2 normal1 = MathUtils.MulT(ref xf2.q, MathUtils.Mul(ref xf1.q, normals1[edge1]));
