...
namespace VelcroPhysics.Tools.TextureTools
{
    ...
    public sealed class TextureConverter
    {
        ...
        public List<Vertices> DetectVertices()
        {
            ...
            do
            {
                ...
                else if (polygonEntrance.HasValue)
                {
                    ...
                    polygon = new Vertices(CreateSimplePolygon(polygonEntrance.Value, new FVector2(polygonEntrance.Value.X - 1f, polygonEntrance.Value.Y)));
                }
                else
                    break;
        ...
        private bool DistanceToHullAcceptable(Vertices polygon, FVector2 point, bool higherDetail)
        {
            ...
            if (higherDetail)
            {
                for (int i = 0; i < polygon.Count; i++)
                {
                    ...
                    if (LineUtils.DistanceBetweenPointAndLineSegment(ref point, ref edgeVertex1, ref edgeVertex2) <= _hullTolerance || FVector2.Distance(point, edgeVertex1) <= _hullTolerance)
                        return false;
        ...
        private bool SplitPolygonEdge(Vertices polygon, FVector2 coordInsideThePolygon, out int vertex1Index, out int vertex2Index)
        {
            FVector2 slope;
