...
namespace VelcroPhysics.Tools.TextureTools
{
    ...
    public class Terrain
    {
        ...
        public Terrain(World world, FVector2 position, float width, float height)
        {
            World = world;
            Width = width;
            Height = height;
        ...
        public void Initialize()
        {
            ...
            _dirtyArea = new AABB(new FVector2(float.MaxValue, float.MaxValue), new FVector2(float.MinValue, float.MinValue));
        }
        ...
        public void ModifyTerrain(FVector2 location, sbyte value)
        {
            // find local position
            // make position local to map space
            FVector2 p = location - _topLeft;
        ...
        private void GenerateTerrain(int gx, int gy)
        {
            ...
            List<Vertices> polys = MarchingSquares.DetectSquares(new AABB(new FVector2(ax, ay), new FVector2(ax + CellSize, ay + CellSize)), SubCellSize, SubCellSize, _terrainMap, Iterations, true);
            if (polys.Count == 0)
                return;
            ...
            FVector2 scale = new FVector2(1f / PointsPerUnit, 1f / -PointsPerUnit);
