...
namespace VelcroPhysics.Extensions.PhysicsLogics.Explosion
{
    ...
    public sealed class RealExplosion : PhysicsLogic
    {
        ...
        public Dictionary<Fixture, FVector2> Activate(FVector2 pos, float radius, float maxForce)
        {
            AABB aabb;
            ...
            Dictionary<Fixture, FVector2> exploded = new Dictionary<Fixture, FVector2>(shapeCount + containedShapeCount);
            ...
            for (int i = 0; i < valIndex; ++i)
            {
                ...
                FVector2 p1 = pos;
                FVector2 p2 = radius * new FVector2((float)Math.Cos(midpt), (float)Math.Sin(midpt)) + pos;
            ...
            for (int i = 0; i < _data.Count; ++i)
            {
                     ...
                     j += offset)
                {
                    ...
                    for (int x = 0; x < fl.Count; x++)
                    {
                        ...
                        FVector2 vectImp = FVector2.Dot(impulse * new FVector2((float)Math.Cos(j), (float)Math.Sin(j)), -ro.Normal) * new FVector2((float)Math.Cos(j), (float)Math.Sin(j));
