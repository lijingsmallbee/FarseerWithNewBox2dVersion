...
namespace VelcroPhysics.Extensions.PhysicsLogics.Explosion
{
    ...
    public sealed class SimpleExplosion : PhysicsLogic
    {
        ...
        public Dictionary<Body, FVector2> Activate(FVector2 pos, float radius, float force, float maxForce = float.MaxValue)
        {
            HashSet<Body> affectedBodies = new HashSet<Body>();

            AABB aabb;
            aabb.LowerBound = pos - new FVector2(radius);
            aabb.UpperBound = pos + new FVector2(radius);
        ...
        private Dictionary<Body, FVector2> ApplyImpulse(FVector2 pos, float radius, float force, float maxForce, HashSet<Body> overlappingBodies)
        {
            Dictionary<Body, FVector2> forces = new Dictionary<Body, FVector2>(overlappingBodies.Count);

            foreach (Body overlappingBody in overlappingBodies)
            {
                if (IsActiveOn(overlappingBody))
                {
                    float distance = FVector2.Distance(pos, overlappingBody.Position);
                    float forcePercent = GetPercent(distance, radius);

                    FVector2 forceVector = pos - overlappingBody.Position;
